...
The loadbalancer only needs access to the TiDB servers (TCP port 4000 by default) and doesn't need access to PD or TiKV servers. When using the HTTP status access to TCP port 10080 is also required.
...
Health checks that check for `read_only` or other MySQL or InnoDB variables should be avoided.
